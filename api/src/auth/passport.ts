...
passport.deserializeUser(async (id: string, done) => {
  try {
    const user = await queryOne<AppUser>(
      'SELECT id, email, name, provider FROM auth.users WHERE id = $1',
      [id]
    );
...
export async function upsertBypassUser(): Promise<AppUser> {
  const existing = await queryOne<AppUser>(
    'SELECT id, email, name, provider FROM auth.users WHERE email = $1',
    ['bob@local.dev']
  );
    ...
    new GoogleStrategy(
      ...
      async (_accessToken, _refreshToken, profile, done) => {
        try {
          ...
          const existing = await queryOne<AppUser>(
            'SELECT id, email, name, provider FROM auth.users WHERE provider = $1 AND provider_id = $2',
            ['google', profile.id]
          );
