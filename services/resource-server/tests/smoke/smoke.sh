...
# resource-server smoke test: ensures at least one healthy target and that GET /resource returns 200
ENV=${ENV:-test}
TG_NAME="resource-server-tg-${ENV}"
PATH_TO_CHECK="/resource"
...
URL="http://$ALB_DNS${PATH_TO_CHECK}"
echo "Requesting $URL"
